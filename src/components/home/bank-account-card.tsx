...
export function BankAccountCard({ account }: BankAccountCardProps) {
  ...
  return (
    <Panel className="flex gap-3" padding="sm">
      ...
      <div className="min-w-0 flex-1">
        <div className="flex items-baseline justify-between gap-2">
          ...
          <span className="text-[13.5px] font-medium tabular-nums whitespace-nowrap text-accent">
            {formatAmount(account.amount)}
          </span>
        </div>
        <p className="mt-0.5 text-[11.5px] text-muted">{account.fundName}</p>
        <code className="font-mono text-xs text-ink">{account.accountNumber}</code>
      </div>
    </Panel>
  )
}
